...
;; Testing apparatus for assignment 3
...
(define (make-test exp ext)
  (match ext
         ["scm"
          (lambda ()
            (define ir (desugar exp))
            (define t0 (test-alphatize assignment-convert alphatize ir))
            (if t0
                (let* ([alpha (alphatize (assignment-convert ir))]
                       [t1 (test-anf-convert anf-convert alpha)])
                  (if t1
                      (test-cps-convert cps-convert (anf-convert alpha))
                      #f))
                #f))]
         ["ir"
          (lambda ()
            (test-alphatize assignment-convert alphatize exp))]
         ["alpha"
          (lambda ()
            (test-anf-convert anf-convert exp))]
         ["anf"
          (lambda ()
            (test-cps-convert cps-convert exp))]))
...
(define (tests-list dir)
  (map
   ...
   (filter (lambda (path)
             ...
             (member (last (string-split p ".")) '("ir" "alpha" "anf" "scm")))
...
(define ((path->test type) p)
  ...
  `(,(last (string-split (string-join (drop-right (string-split (path->string p) ".") 1) ".") "/"))
    ,type
    ,(make-test (with-input-from-file p read #:mode 'text)
         ...
         [(list "mk-test-props")
          ...
          (for-each
           ...
           '("build.language=c"
             ...
             "test.exec=timeout -s KILL 5s /usr/local/bin/racket ./tests.rkt &"))
          (for-each
           (lambda (kv)
             (match kv
                    [(cons grp ts)
