...
; prims removed by simplify-prims are commented again afterward (but must be kept around for desugar)
(define prims-list '(= < <= + - * / >= > ; >= >  
                     cons? null? cons car cdr list first second third fourth last
                     length list-tail drop take member memv append map foldl foldr
                     vector? list->vector make-vector vector-ref vector-set! vector-length vector vector
                     set set->list list->set set-add set-union set-first set-rest set-remove
                     hash-ref hash-set hash-count hash-has-key? hash? hash hash
                     void? number? integer? procedure? list?
                     void print halt
                     eq? eqv? equal? not))
...
(define (eval-llvm llvm-str)
  ...
  ;(system (string-append "clang++ combined.ll " libgc-path " -I " gc-include-path " -lpthread -o bin"))
  (system (string-append "clang++ combined.ll " " -o bin"))
